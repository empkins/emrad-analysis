...
class BiLSTM(Algorithm):
    ...
    def self_optimize(self, training_data: np.ndarray, labels: np.ndarray):
        ...
        assert self._model.layers[0].input_shape[1] == training_data.shape[
            1], f"Your training data has dimension {training_data.shape} while the model has input shape {self._model.layers[0].input_shape}!"
        assert self._model.layers[0].input_shape[2] == training_data.shape[
            2], f"Your training data has dimension {training_data.shape} while the model has input shape {self._model.layers[0].input_shape}!"
    ...
    def predict(self, input_data: np.ndarray):
        ...
        assert self._model is not None, \
            ("Before making a prediction you will have to train a model "
             "using the self.self_optimize method of the same instance!")
