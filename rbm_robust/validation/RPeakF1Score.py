...
class RPeakF1Score(Algorithm):
    ...
    def compute_predictions(self, predicted_r_peak_signal: np.ndarray, ground_truth_r_peak_signal: np.ndarray):
        ...
        if len(pred_peaks) == 0:
            ...
            self.pred_peaks_ = 0
            return self
        print(f"Length of pred_peaks array (in F1 scoring function) is {len(pred_peaks)}")
        ...
        while next_gt < len(gt_peaks):
            ...
            next_gt += 1
        self.tp_ = true_positives
        self.pred_peaks_ = len(pred_peaks)
        self.total_peaks_ = len(gt_peaks)
        return self
